    ...
    for j in range(grid_cell_array.get_length()):
        if grid_cell_array.get_cell_attr(j, "start_index") != None:
            ...
            if grid_cell_array.get_cell_attr(j, "start_index") == 0:
                m_occ_pred = weight_array_accum[grid_cell_array.get_cell_attr(
                    j, "end_index")]
            else:
                m_occ_pred = weight_array_accum[grid_cell_array.get_cell_attr(j, "end_index")] - \
                    weight_array_accum[grid_cell_array.get_cell_attr(
                        j, "start_index") - 1]
            ...
            if (m_occ_pred > particle_array.p_S):
                ...
                particle_array.normalize_p_S(grid_cell_array.get_cell_attr(
                    j, "start_index"), grid_cell_array.get_cell_attr(j, "end_index"))

            # predicted free mass
            m_free_pred = predict_free_mass(
                grid_cell_array.get_cell_attr(j, "m_free"), m_occ_pred, alpha)
            ...
            if check_values and (m_occ_up > 1 or m_occ_up < 0):
                ...
                grid_cell_array.get_cell_attr(j, "end_index"), "mass_occ: ", m_occ_up, "mass_free: ", \
                    ...
                    "weight[end]: ", weight_array_accum[grid_cell_array.get_cell_attr(
                        j, "end_index")]
            ...
            if check_values:
                assert(all([rho_b >= 0., rho_p >= 0., m_free_up >= 0.,
                            m_occ_up >= 0., m_occ_up <= 1., m_free_up <= 1.]))

            grid_cell_array.store_cell_values(
                j, rho_b, rho_p, m_free_up, m_occ_up)
...
def update_of(m_occ_pred, m_free_pred, meas_m_occ, meas_m_free):
    ...
    m_occ_up = (m_occ_pred * meas_cell_unknown + m_unknown_pred *
                meas_m_occ + m_occ_pred * meas_m_occ) / (1. - K)
    m_free_up = (m_free_pred * meas_cell_unknown + m_unknown_pred *
                 meas_m_free + m_free_pred * meas_m_free) / (1. - K)
...
def separate_newborn_part(m_occ_pred, m_occ_up, p_B):
    ...
    rho_b = (m_occ_up*p_B*(1. - m_occ_pred)) / \
        (m_occ_pred + p_B*(1. - m_occ_pred))
