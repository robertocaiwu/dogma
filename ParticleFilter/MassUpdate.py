...
def MassUpdate(meas_cell_array, grid_cell_array, p_B, alpha, check_values=False):
    ...
    for j in range(grid_cell_array.get_length()):
        ...
        m_occ_pred = predict_mass(grid_cell_array.get_cell_attr(
            j, "m_occ"), grid_cell_array.get_cell_attr(j, "m_free"), alpha)

        # predicted free mass
        m_free_pred = predict_mass(
            grid_cell_array.get_cell_attr(j, "m_free"), m_occ_pred, alpha)

        # debugging tests
        if check_values and (m_occ_pred > 1 or m_occ_pred < 0):
            print("start index: %f;" % grid_cell_array.get_cell_attr(j, "start_index"))
            print("end index: %f;" % grid_cell_array.get_cell_attr(j, "end_index"))
            print("mass_occ: %f;" % (m_occ_pred))
            print("mass_free: %f" % (m_free_pred))

        if (m_occ_pred > 1.0):
            print("This is m_occ_pred: %f" % m_occ_pred)
        assert (m_occ_pred <= 1.)
        assert (m_occ_pred >= 0.)
        ...
        if check_values and (m_occ_up > 1.001 or m_occ_up < 0.):
            print("mass_occ: {}; mass_free: {}".format(m_occ_up, m_free_up))
