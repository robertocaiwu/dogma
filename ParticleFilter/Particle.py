...
class Particle(object):
    ...
    def __init__(self, position, index, weight, state_size=4, p_S=0.99, scale_vel=5., scale_acc=2.,
        ...
        if self.state_size == 4:
            ...
            self.state[2] = np.random.normal(
                loc=0.0, scale=self.scale_vel, size=None)
            self.state[3] = np.random.normal(
                loc=0.0, scale=self.scale_vel, size=None)
        else:
    ...
    def persist_update_weight(self):
        ...
        if (self.weight < 0.):
            print(
                "ATTENTION: persistent update has negative weights in particle prediction")
        return

    def normalize(self, updated_weight, p_A, mu_A, mu_UA):
        self.weight = p_A * updated_weight * \
            mu_A + (1. - p_A) * self.weight * mu_UA
        return
...
class ParticleArray(object):
    ...
    def predict(self, particle_index, zeta):
        ...
        self.particles[particle_index].predict(
            transition_matrix=self.transition_matrix, zeta=zeta)
    ...
    def accumulate_weight(self):
        ...
        for i in range(1, len(accumulated_array)):
            ...
            accumulated_array[i] = accumulated_array[i-1] + \
    ...
    def accumulate_cell_state(self, grid_cell_array, index_in_state):
        ...
        for i in range(grid_cell_array.get_length()):
            ...
            cell_accumulation = 0.
            start_particle_index = grid_cell_array.get_cell_attr(
                i, "start_index")
            ...
            if start_particle_index != None:
                ...
                for j in range(start_particle_index, end_particle_index + 1):
                    if self.get_weight(j) < 0.:
                        print("THIS IS AN ISSUE %5.3f" % self.get_weight(j))
                    cell_accumulation += self.get_state(
                        j)[index_in_state] * self.get_weight(j)
    ...
    def initialize_newborn_A(self, particle_index, measurement, position=(0., 0.)):
        ...
        newstate = np.pad(
            position, (0, self.state_size - len(position)), 'constant')
        ...
        if self.state_size == 4:
            newstate[2] = np.random.normal(
                loc=0.0, scale=self.scale_vel, size=None)
            newstate[3] = np.random.normal(
                loc=0.0, scale=self.scale_vel, size=None)
        else:
            raise Exception("Unexpected state size.")
    ...
    def normalize_weight(self, updated_weight, grid_cell_array, meas_cell_array):
        for i in range(self.get_length()):
            ...
            mu_UA = grid_cell_array.get_cell_attr(
                self.get_cell_idx(i), "mu_UA")
    ...
    def normalize_p_S(self, start_index, end_index):
        ...
        for i in range(start_index, end_index + 1):
            self.particles[i].weight = self.particles[i].weight / \
                particle_sum * self.p_S
